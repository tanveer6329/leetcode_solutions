...
int countPrimes(int n) {
    ...
    for (int p = 3; p * p < n; p += 2) {
        if (is_prime[p / 2]) { // Check the corresponding index for p
            ...
            // multiple += p * 2 ensures that only odd multiples of p are considered, skipping even multiples entirely.
...
int main() {
    ...
    cout << "Number of prime numbers less than " << n << ": " << prime_count << endl;
